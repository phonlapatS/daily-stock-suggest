...
class HongKongLogicThresholdOnlyEngine(BasePatternEngine):
    """
    Hong Kong Market Engine - Logic Threshold Only
    ...
    def __init__(self):
        super().__init__()
        # Hong Kong market specific settings
        self.min_floor = 0.007  # 0.7% minimum move for Hong Kong market (fractional)
        
    def analyze(self, df, settings):
        """
        Analyze Hong Kong stocks with threshold-only logic.
        ...
        # Rule 3: Best Fit Selection (Count >= 15 for HK)
        best_fit = self.select_best_fit(
            df['close'], pct_change, effective_std, active_pattern,
            min_count=15, direction_override=direction
        )
        ...
        is_tradeable = self.check_trustworthy(stats, 60.0, 15)
        
        return [{
            'engine': 'HONGKONG_LOGIC_THRESHOLD_ONLY',
