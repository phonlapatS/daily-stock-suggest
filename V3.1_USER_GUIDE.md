...
- **Prob**: Probability of the *predicted direction*.
- **Chance**: The predicted direction (🟢 UP or 🔴 DOWN).
- **Avg_Ret**: Expected return if the prediction holds.

### Part 2: Streak Profile (Momentum Health)
Focuses on **Trend Duration**.
- **Day**: Current length of the streak (e.g., 3 days UP).
- **Prediction**: Probability of the streak *continuing* to the next day.
  - `🟢 CONT.`: Likely to continue.
  - `🔴 REV.`: Likely to reverse (end).
- **Avg_Ints**: How intense the current streak is.
