...
def calculate_stats_from_log(log_file="logs/performance_log.csv"):
    ...
    for market in markets:
        ...
        for _, r in stock_df.iterrows():
            print(f"{r['Stock']:<12} {r['Trades']:<8} {r['W/L']:<12} {r['Winrate']:>7.0f}% {r['Avg Win']:>9.1f}% {r['Avg Loss']:>9.1f}%")
    ...
    n_g_total = len(completed_df)
    
    g_winrate = (n_g_prof / n_g_total * 100) if n_g_total > 0 else 0
    g_avg_win = g_profits['P/L'].mean() if n_g_prof > 0 else 0
    g_avg_loss = g_losses['P/L'].mean() if n_g_loss > 0 else 0
    
    print(f"{'Type':<12} {'#':<8} {'Avg P/L %':<15} {'Total P/L %':<15} {'Winrate':<15}")
    print("-" * 115)
    print(f"{'Profit':<12} {n_g_prof:<8} {g_avg_win:<15.2f} {g_profits['P/L'].sum():<15.2f} {g_winrate:<15.1f}")
    print(f"{'Loss':<12} {n_g_loss:<8} {g_avg_loss:<15.2f} {g_losses['P/L'].sum():<15.2f} {'':<15}")
    print("-" * 115)
    print(f"✅ Performance check completed. Global Accuracy/Winrate: {g_winrate:.1f}%")
