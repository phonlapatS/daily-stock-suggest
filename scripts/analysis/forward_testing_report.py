...
def _load_log() -> pd.DataFrame:
    ...
    for c in ("prob", "stats", "price_at_scan", "price_actual", "correct", "avg_return"):
        if c in df.columns:
...
def _print_report(df: pd.DataFrame, days: int) -> None:
    ...
    if "pnl_pct" in verified.columns:
        ...
        # Format for display
        verified["forecast"] = verified["forecast"].apply(lambda x: "+" if x == "UP" else "-")
        verified["actual"] = verified["actual"].apply(lambda x: "+" if x == "UP" else ("-" if x == "DOWN" else "SIDE"))
        verified["correct"] = verified["correct"].apply(lambda x: "✅" if x == 1 else "❌")
        
        disp_cols = ["scan_date", "symbol", "exchange", "forecast", "actual", "move_pct", "avg_return", "prob", "correct"]
        top = verified.sort_values("move_pct", ascending=False).head(10)[disp_cols]
        bot = verified.sort_values("move_pct", ascending=True).head(10)[disp_cols]
        
        print("\nTop 10 Move% (Actual):")
        print(top.to_string(index=False, formatters={"move_pct": "{:+.2f}%".format, "avg_return": "{:+.2f}%".format}))
        print("\nBottom 10 Move% (Actual):")
        print(bot.to_string(index=False, formatters={"move_pct": "{:+.2f}%".format, "avg_return": "{:+.2f}%".format}))
