#!/usr/bin/env python
"""
Clean Taiwan Cache - ลบ cache สำหรับ Taiwan market
"""
...
def clean_taiwan_cache():
    """ลบ cache สำหรับ Taiwan market"""
    
    print("="*80)
    print("Clean Taiwan Cache - ลบ cache สำหรับ Taiwan Market")
    print("="*80)
    print()
    
    deleted_count = 0
    
    # 1. ลบ cache files สำหรับ TWSE (Taiwan stocks)
    ...
    if os.path.exists(cache_dir):
        ...
        if cache_files:
            ...
            print("ℹ️  ไม่พบ TWSE cache files")
    
    # 2. ลบ trade_history_TAIWAN.csv
    ...
    # 3. ทำความสะอาด full_backtest_results.csv (ลบ entries ที่เป็น Taiwan)
    ...
    if os.path.exists(full_results_file):
        try:
            ...
            # Filter out Taiwan entries
            if 'group' in df.columns:
                df_cleaned = df[~df['group'].str.contains('TAIWAN|TW', case=False, na=False)]
                ...
                if deleted_entries > 0:
                    ...
                    print(f"ℹ️  ไม่มี Taiwan entries ใน {full_results_file}")
        except Exception as e:
            print(f"❌ ไม่สามารถทำความสะอาด {full_results_file}: {e}")
    else:
        print(f"ℹ️  ไม่พบ: {full_results_file}")
