...
## 🔧 V3.3 → V3.4 Change Log

### 📅 Day 1: V3.3 Completion (2026-02-04)

#### 4-Table Report System
| Change | File | Description |
|--------|------|-------------|
| Table 1: Thai Strict | `scripts/calculate_metrics.py` | Prob > 60%, RR > 2.0 |
| Table 2: Thai Balanced | `scripts/calculate_metrics.py` | Prob > 60%, 1.5 < RR ≤ 2.0 |
| Table 3: International Observation | `scripts/calculate_metrics.py` | Prob > 55%, RR > 1.1 |
| Table 4: International Sensitivity | `scripts/calculate_metrics.py` | Prob > 50%, 0.5 < RR ≤ 1.1 |

#### Report Enhancements
| Change | File | Description |
|--------|------|-------------|
| Symbol Mapping | `scripts/calculate_metrics.py` | 9988 → ALIBABA, 600519 → MOUTAI |
| Country Codes | `scripts/calculate_metrics.py` | TH, US, CN, HK, TW |
| Min Signal ≥ 30 | `scripts/calculate_metrics.py` | Filter low-count patterns |
| Integer Formatting | `scripts/calculate_metrics.py` | Signals column as int |

---

### 📅 Day 2: V3.4 Development (2026-02-04 ~ 2026-02-05)
...
| Min Data 1000 → 250 | `scripts/backtest.py` | Support newer stocks |
...
| Change | File | Description |
|--------|------|-------------|
| 5,000-Bar Backtest | `scripts/backtest.py` | ✅ **Verified** (SUPER 70%, TPIPP 63%) |
| Threshold Tuning | `config.py` | ✅ **Finalized** (Targeted Strategies) |
| 4-Table Report | `scripts/calculate_metrics` | ✅ **Production Ready** |
...
| Change | File | Description |
|--------|------|-------------|
| Sensitivity Matrix | `scripts/test_sensitivity.py` | ✅ **Analysis** (Threshold 0.6-0.7 / Days 30) |
| Equity Curve | `scripts/visualize_equity.py` | ✅ **Proof** (TPIPP vs SIRI vs Market) |
| Final Report | `docs/THRESHOLD_SENSITIVITY_REPORT_V3.4.md` | ✅ **Documentation** (Strategy Finalized) |

---

## 🚀 Future Roadmap: V4.0 (Real-time Alert)

| # | Task | Priority | Description |
|---|------|----------|-------------|
| 1 | **Line/Telegram Bot** | 🔴 HIGH | Push signals to mobile in real-time |
| 2 | **Dashboard UI** | 🟡 MEDIUM | Web Interface for Equity Curve monitoring |
| 3 | **Automated Execution** | 🔵 LOW | Connect API for auto-trade (Experimental) |

---

## 🎓 Key Learnings (V3.4 Final)
